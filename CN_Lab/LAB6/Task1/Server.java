...
public class Server {
    ...
    // Decode 2D matrix bits back into registration string
    ...
    public static void main(String[] args) {
        try (ServerSocket serverSocket = new ServerSocket(5001)) {
            ...
            try (
                ...
                int rows = (data.length() / 9) - 1;  // last row reserved for parity
                int cols = 8;  // 8 bits (1 byte per character)
                int[][] matrix = new int[rows + 1][cols + 1];

                // Convert received string into matrix
                int idx = 0;
                for (int i = 0; i <= rows; i++) {
                    for (int j = 0; j <= cols; j++) {
